...
namespace Common
{
    public class Cs_XLSX
    {
        public void ExportarParaExcel<T>(List<T> _Lentidade, string PathArquivo)
        {
            ...
            foreach (var p in props)
            {
                ...
                if (info.PropertyType.Name.Equals("String") || info.PropertyType.Name.Equals("Char") || info.PropertyType.Name.Equals("Guid") || info.PropertyType.Name.Equals("Object"))
                    TipoColuna[((AtributoXLS)Obj.GetValue(0)).Ordem] = "string";
                else if ((info.PropertyType.Name.Equals("Int32")) || (info.PropertyType.Name.Equals("Int64")))
                    TipoColuna[((AtributoXLS)Obj.GetValue(0)).Ordem] = "Int";
                    ...
                    TipoColuna[((AtributoXLS)Obj.GetValue(0)).Ordem] = "decimal";
                else if (info.PropertyType.Name.Equals("DateTime"))
                    TipoColuna[((AtributoXLS)Obj.GetValue(0)).Ordem] = "DateTime";
                else if (info.PropertyType.Name.Equals("bool"))
                    TipoColuna[((AtributoXLS)Obj.GetValue(0)).Ordem] = "bool";


                for (int i = 0; i < _Lentidade.Count; i++)
                {
                    T _entidade = _Lentidade[i];
                    arrHeader[i, ((AtributoXLS)Obj.GetValue(0)).Ordem] = Convert.ToString(p.GetValue(_entidade, null));
                }
            }
            ...
            #region Gerar Xml
            {
                ...
                try
                {
                    ...
                    //Microsoft.Office.Interop.Excel.Range excelCell = (Microsoft.Office.Interop.Excel.Range)excelWorksheet.get_Range("B4:FZ4", Type.Missing); //Select a range of cells
                    //Microsoft.Office.Interop.Excel.Range excelCell1 = (Microsoft.Office.Interop.Excel.Range)excelWorksheet.get_Range("B4:B4", Type.Missing); //Select a single cell
        ...
        public List<T> RetornaDR<T>(string PathArquivo)
        {
