        ...
        public void ExportarParaExcel<T>(List<T> _Lentidade, string PathArquivo)
        {
            ...
            foreach (var p in props)
            {
                ...
                if (info.PropertyType.Name.Equals("String") || info.PropertyType.Name.Equals("Char") || info.PropertyType.Name.Equals("Guid") || info.PropertyType.Name.Equals("Object"))
                    TipoColuna[((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "string";
                else if ((info.PropertyType.Name.Equals("Int32")) || (info.PropertyType.Name.Equals("Int64")))
                    TipoColuna[((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "Int";
                    ...
                    TipoColuna[((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "decimal";
                else if (info.PropertyType.Name.Equals("DateTime"))
                    TipoColuna[((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "DateTime";
                else if (info.PropertyType.Name.Equals("bool"))
                    TipoColuna[((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "bool";


                for (int i = 0; i < _Lentidade.Count; i++)
                {
                    T _entidade = _Lentidade[i];
                    arrHeader[i, ((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = Convert.ToString(p.GetValue(_entidade, null));
                }
            }
            ...
            #region Gerar Xml
            {
                ...
                try
                {
                    ...
                    excelWorksheet = (Microsoft.Office.Interop.Excel.Worksheet)wb.Sheets[1]; //Select the first sheet
                    ...
                    for (int Coluna = 0; Coluna < TipoColuna.Length; Coluna++)
                    {
                        ...
                        Microsoft.Office.Interop.Excel.Range excelCell = excelWorksheet.get_Range(ConvertNumAlfa(Coluna) + "2:" + ConvertNumAlfa(Coluna) + Convert.ToString(2 + _Lentidade.Count - 1), Type.Missing); //Select a single cell
        ...
        public List<T> RetornaEntidade<T>(string PathArquivo)
        {
            ...
            foreach (PropertyInfo pi in Activator.CreateInstance(typeof(T)).GetType().GetProperties())
            {
                foreach (object objAux in tipo.GetProperty(pi.Name).GetCustomAttributes(true))
                {
                    ...
                    {
                        if (MaxIndice < ((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem)
                            MaxIndice = ((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem;
                        ObjIndices.Add(pi.Name, ((ControleFinanceiro.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem);
                    }
                    else if (objAux.GetType().Name == "AtributoXLS")
                    {
                        InicioLinha = ((ControleFinanceiro.DTO.XLSX_Planilha.AtributoXLS)objAux).Linha + 1;
                        InicioColuna = ((ControleFinanceiro.DTO.XLSX_Planilha.AtributoXLS)objAux).Coluna;
                    }
                }
            ...
            try
            {
                ...
                //To read cell(s) in the worksheet,
                excelWorksheet = (Microsoft.Office.Interop.Excel.Worksheet)wb.Sheets[1]; //Select the first sheet
                Microsoft.Office.Interop.Excel.Range excelCell2 = (Microsoft.Office.Interop.Excel.Range)excelWorksheet.get_Range(string.Format("{0}{1}:{0}{1}", ConvertNumAlfa(InicioColuna), InicioLinha), Type.Missing);

                int intLinhas = 0;
                for (int i = 1; ((Microsoft.Office.Interop.Excel.Range)excelCell2.Cells[i, 1]).Value2 != null; i++)
                {
                    intLinhas++;
                }

                Microsoft.Office.Interop.Excel.Range excelCell = excelWorksheet.get_Range(string.Format("{0}{2}:{1}{3}", ConvertNumAlfa(InicioColuna), ConvertNumAlfa(InicioColuna + MaxIndice), InicioLinha, InicioLinha + intLinhas), Type.Missing); //Select a single cell
